...
# dash-improve-my-llms installs from PyPI as of 2.3.2. vendor/ still holds
# dash_clerk_auth (not on PyPI, deliberately outside requirements.txt) so the
# optional-auth install command in docs/authentication works inside the image.
COPY vendor/ ./vendor/
