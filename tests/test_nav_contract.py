...
def test_api_page_is_not_registered_when_no_package_is_declared(app_module):
    import dash

    from lib.constants import API_PACKAGES

    assert API_PACKAGES == [], "the template documents no component package"
    assert "/api" not in [p["path"] for p in dash.page_registry.values()]
