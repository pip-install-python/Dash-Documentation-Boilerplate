...
# single-H1 shape.
# 2.8.0 is the ledger floor (2026-08-29): ONE classifier — `classify()`
# is the registry robots.txt is rendered from, and lib/analytics_tracker
# delegates to it instead of carrying a fourth UA list that filed ClaudeBot
# as search; the READ EVENT — `on_document_read` hands the app one row per
# corpus document served (tier, verdict, bytes, verified vendor), which
# the tracker keeps as the ledger's `reads` table; `Vary: User-Agent` on
# the lane-split responses; and verified vendor identity (`verified` is
...
# 2026-08-29). Through 1.6.36 this host blocked the AI-training crawlers
...
# corpus (/llms.txt and the tiers) was open — a wall that decided by
# vendor class what nobody could account for. The ledger changed that:
# since 1.6.34 every corpus read is a row (tier, vendor, verified, bytes)
# and the hub reconciles it against the wire (boilerplate 534 == 534 on
# 2026-08-29). A read that is recorded and priceable does not need a
# wall; it needs a policy. So training crawlers are ALLOWED by default,
# the same as search fetchers and traditional bots, and the per-vendor
# knob is the tool from here on — block or meter ONE vendor by name when
# its ledger rows justify it, never the whole class:
#
#     vendor_policy={"bytespider": "block", "gptbot": "meter"}
#
# (2.3.3's per-vendor buckets still matter: they are what makes a
# per-vendor line mean the vendor it names.) A fork whose posture fence
# declares ai_bots 403 by design — clerkhook — keeps block_ai_training=True
# and says so in its DIVERGENCES.md; item 15 in the sync spec.
